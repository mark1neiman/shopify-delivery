...
function asOrderPaidPayload(payload: unknown): OrderPaidWebhookPayload {
  if (!payload || typeof payload !== "object") return {};
  return payload as OrderPaidWebhookPayload;
}

function detectOrderLocale(payload: OrderPaidWebhookPayload): string {
  return (
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    const payload = asOrderPaidPayload(rawPayload);
    const orderId = normalizeOrderId(payload.admin_graphql_api_id || payload.id);
    if (!orderId) {
      console.warn("[invoice] orders/paid webhook missing order id, skipping");
      return new Response();
    }
    ...
    const result = await enqueueInvoiceJob({
      shop,
      triggerTopic: String(topic || "orders/paid"),
      orderId,
